...
class _InactiveState extends State<Inactive> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text("In-Active",style: GoogleFonts.rubik(fontWeight: FontWeight.w500),
        ),
  ...
  inactivevacancies(context) async{
    HashMap<String, String> map = HashMap();
    map["updte"] = "1";
    map["user_id"]= user_id;


    var res = await http.post(Uri.parse("$mainurl/inactive_vacancies.php"),
        body: jsonEncode(map));
    print(res.body);
